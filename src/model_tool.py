...
class ToxModel():
    ...
    def load_model_from_name(self, model_name):
        self.model = load_model(os.path.join(
            self.model_dir,
            '%s_model.h5' % model_name))
        self.tokenizer = cPickle.load(open(os.path.join(
            self.model_dir, '%s_tokenizer.pkl' % model_name), 'rb'))
        with open(os.path.join(
                self.model_dir, '%s_hparams.json' % self.model_name),
                'r') as f:
    ...
    def fit_and_save_tokenizer(self, texts):
        ...
        cPickle.dump(self.tokenizer, open(
    ...
    def train(
        ...
        if self.hparams['stop_early']:
            callbacks.append(EarlyStopping(
                ...
                monitor='val_loss', patience=self.hparams['es_patience'],
    ...
    def build_conv_layer(
        ...
        output = Conv1D(
                filter_size,
                kernel_size, activation='relu', padding='same')(input_tensor)
        if pool_size:
